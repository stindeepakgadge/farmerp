Param(
    ...
    [parameter(Mandatory=$false)][string]$appName="farmizo",
    ...
    [parameter(Mandatory=$false)][string]$tlsSecretName = "farmizo-tls-custom",
...
function Install-Chart  {
    ...
    if ($customRegistry) {
        $options = "$options --set inf.registry.server=$registry --set inf.registry.login=$dockerUser --set inf.registry.pwd=$dockerPassword --set inf.registry.secretName=farmizo-docker-scret"
    }
    
    if ($chart -ne "farmizo-common" -or $customRegistry)  {       # farmizo-common is ignored when no secret must be deployed        
...
if ($sslSupport -eq "staging") {
    $sslEnabled=$true
    $tlsSecretName="farmizo-letsencrypt-staging"
    $sslIssuer="letsencrypt-staging"
}
elseif ($sslSupport -eq "prod") {
    $sslEnabled=$true
    $tlsSecretName="farmizo-letsencrypt-prod"
...
if ($clean) {    
    $listOfReleases=$(helm ls --filter farmizo -q)    
...
Write-Host "Begin farmizo installation using Helm" -ForegroundColor Green

$infras = ("sql-data", "nosql-data", "rabbitmq", "keystore-data", "basket-data")
$charts = ("farmizo-common", "basket-api","catalog-api", "identity-api", "locations-api", "marketing-api", "mobileshoppingagg","ordering-api","ordering-backgroundtasks","ordering-signalrhub", "payment-api", "webmvc", "webshoppingagg", "webspa", "webstatus", "webhooks-api", "webhooks-web")
...
if ($deployInfrastructure) {
    ...
    Write-Host "farmizo infrastructure (bbdd, redis, ...) charts aren't installed (-deployCharts is false)" -ForegroundColor Yellow
}
...
if ($deployCharts) {
    ...
    Write-Host "farmizo non-infrastructure charts aren't installed (-deployCharts is false)" -ForegroundColor Yellow
}
