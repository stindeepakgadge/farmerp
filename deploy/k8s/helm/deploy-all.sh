...
charts=(farmizo-common apigwwm apigwws catalog-api identity-api marketing-api webmvc webshoppingagg webstatus webhooks-api webhooks-web)
...
do
    ...
    if [[ $use_custom_registry ]]; then 
      helm install "$app_name-$chart" --namespace $namespace --set "ingress.hosts={$dns}" --set inf.registry.server=$container_registry --set inf.registry.login=$docker_username --set inf.registry.pwd=$docker_password --set inf.registry.secretName=farmizo-docker-scret --values app.yaml --values inf.yaml --values $ingress_values_file --set app.name=$app_name --set inf.k8s.dns=$dns --set image.tag=$image_tag --set image.pullPolicy=Always $chart 
