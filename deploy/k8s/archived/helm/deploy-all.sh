...
Parameters:
  ...
  -n | --app-name <the name of the app>
    Specifies the name of the application (default: farmizo).
  --namespace <namespace name>
    Specifies the namespace name to deploy the app. If it doesn't exists it will be created (default: farmizo).
...
It is recommended that you check your selected namespace, 'farmizo' by default, is already in use.
...
if [[ $build_solution ]]; then
  ...
  dotnet publish -o obj/Docker/publish ../../farmizo.sln
fi
...
if [[ $push_images ]]; then
  ...
  if [[ -z "$(docker image ls -q --filter=reference=farmizo/$service:$image_tag)" ]]; then
  ...
  do
    ...
    docker tag "farmizo/$service:$image_tag" "$container_registry/$service:$image_tag"
...
charts=(farmizo-common apigwmm apigwms apigwwm apigwws basket-api catalog-api identity-api locations-api marketing-api mobileshoppingagg ordering-api ordering-backgroundtasks ordering-signalrhub payment-api webmvc webshoppingagg webspa webstatus webhooks-api webhooks-web)
...
do
    ...
    if [[ $use_custom_registry ]]; then 
      helm install --namespace $namespace --set "ingress.hosts={$dns}" --set inf.registry.server=$container_registry --set inf.registry.login=$docker_username --set inf.registry.pwd=$docker_password --set inf.registry.secretName=farmizo-docker-scret --values app.yaml --values inf.yaml --values $ingress_values_file --set app.name=$app_name --set inf.k8s.dns=$dns --set image.tag=$image_tag --set image.pullPolicy=Always --name="$app_name-$chart" $chart 
    elif [[ $chart != "farmizo-common" ]]; then  # farmizo-common is ignored when no secret must be deployed
